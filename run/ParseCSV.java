...
public final class ParseCSV {
    ...
    /**
     *
     * @param s, type: String, from *.csv file
     * @return parsing Route from *.csv file
     */
    public static Route parse_csv_to_route(String s){
        ...
        java.util.Date creationDate = new Date(Long.parseLong(parse_args[14]));
        Location location_from;
        Location location_to;
        boolean ok = "class RouteDir.IntLocation".equals(parse_args[5]);
        if (ok){location_from=new IntLocation(Integer.parseInt(parse_args[6]),
                Long.parseLong(parse_args[7]),parse_args[8] );}else
                    if("class RouteDir.Location".equals(parse_args[5])){location_from=null;} else {location_from=new FloatLocation(Float.parseFloat(parse_args[6]),
                Float.parseFloat(parse_args[7]),Integer.parseInt(parse_args[8]) );}
        ok = "class RouteDir.IntLocation".equals(parse_args[9]);
        if (ok){ location_to=new IntLocation(Integer.parseInt(parse_args[10]),
                Long.parseLong(parse_args[11]),parse_args[12] );}else {location_to=new FloatLocation(Float.parseFloat(parse_args[10]),
